...
const apply=()=>{
 ...
 .gg-hist{white-space:nowrap}.gg-hist:before{content:"> ";color:#00ff46}`;
 root.appendChild(style);
 const fixAnimals=()=>{const a=root.querySelector('#animal');if(!a)return false;const current=a.value;const html=ANIMALS.map((name,id)=>`<option value="${id}">${id} • ${name}</option>`).join('');if(a.innerHTML!==html)a.innerHTML=html;if([...a.options].some(o=>o.value===current))a.value=current;else a.value='0';return true};
 fixAnimals();
 const animalWatch=new MutationObserver(()=>fixAnimals());animalWatch.observe(body,{childList:true,subtree:true});
 let box=root.querySelector('#gg-term-extra');
 if(!box){box=document.createElement('div');box.id='gg-term-extra';box.innerHTML='<button id="gg-stat">[SYS] STATUS</button><button id="gg-storage">[SYS] STORAGE</button><button id="gg-garden">[RUN] GARDEN</button><button id="gg-inventory">[RUN] INVENTORY</button>';const tabs=body.querySelector('.tabs');body.insertBefore(box,tabs||null)}
 ...
 if(input&&!input.dataset.ggHistory){input.dataset.ggHistory='1';const hist=[];let pos=-1;input.addEventListener('keydown',e=>{if(e.key==='Enter'){const v=input.value.trim();if(v){hist.push(v);pos=hist.length;add(v)};return}if(e.key==='ArrowUp'){if(!hist.length)return;e.preventDefault();pos=Math.max(0,pos-1);input.value=hist[pos]||''}if(e.key==='ArrowDown'){if(!hist.length)return;e.preventDefault();pos=Math.min(hist.length,pos+1);input.value=hist[pos]||''}})}
 add('terminal extensions loaded');add('animal IDs locked: 0-8');return true;
};
